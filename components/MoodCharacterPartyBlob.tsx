...
export default function MoodCharacterPartyBlob({ mood, isLoading, accentColor }: Props) {
  ...
  return (
    ...
    >
      <svg viewBox="0 0 200 220" width="100%" height="100%" aria-hidden style={{ overflow: "visible" }}>
        ...
        <path
          d="M100 28 C145 28 175 58 175 105 C175 152 145 188 100 195 C55 188 25 152 25 105 C25 58 55 28 100 28 Z"
        ...
        <g transform="translate(100, 118)">
          <BlobEyes expression={expression} />
          <BlobMouth expression={expression} />
        </g>
        {expression === "ecstatic" && (
...
function BlobEyes({ expression }: { expression: MoodExpression }) {
  switch (expression) {
    case "thinking":
      return (
...
function BlobMouth({ expression }: { expression: MoodExpression }) {
